import type { GetServerSideProps, NextPage } from "next"; import Head from "next/head"; import { tmdb } from "../lib/tmdb"; import Navbar from "../components/Navbar"; import Hero from "../components/Hero"; import Row from "../components/Row"; import Footer from "../components/Footer"; import SetupBanner from "../components/SetupBanner";
const DEMO = Array.from({length:20},(_,i)=>({id:i+1,title:["Inception","The Dark Knight","Interstellar","Parasite","Money Heist","Lupin","3 Idiots","Ertuğrul","Barfi!","Pathaan","Dune","Avatar","Oppenheimer","Past Lives","Saltburn","The Bear","Succession","Wednesday","Shogun","Reacher"][i],overview:"Add TMDB_API_KEY to .env.local.",poster_path:null,backdrop_path:null,vote_average:8.1+(i%4)*.2,release_date:`${2020+(i%5)}-01-01`,media_type:i>13?"tv":"movie",genre_ids:[28,12]}));
...
const Home: NextPage<Props> = ({ trending,topMovies,nowPlaying,bollywood,turkish,french,popularTV,korean,topTV,hasTMDB,hasDB }) => (
  <><Head><title>StreamVault — Watch Movies &amp; Series Free 4K</title><meta name="viewport" content="width=device-width,initial-scale=1"/></Head>
    <Navbar/>
...
export const getServerSideProps: GetServerSideProps = async () => {
  const hasTMDB=!!process.env.TMDB_API_KEY; const hasDB=!!process.env.DATABASE_URL;
  ...
  const [tr,top,now,bw,tk,fr,tv,kr,ttv]=await Promise.allSettled([tmdb.trending(),tmdb.topMovies(),tmdb.nowPlaying(),tmdb.bollywood(),tmdb.turkish(),tmdb.french(),tmdb.popularTV(),tmdb.korean(),tmdb.topTV()]);
