...
function embedUrl(pid: string, id: number, s: number, e: number) {
  switch (pid) {
    case "vidsrc":     return `https://vidsrc.to/embed/tv/${id}/${s}/${e}`;
    case "vidsrc2":    return `https://vidsrc.xyz/embed/tv?tmdb=${id}&season=${s}&episode=${e}`;
    ...
    case "multiembed": return `https://multiembed.mov/directstream.php?video_id=${id}&tmdb=1&s=${s}&e=${e}`;
    default:           return `https://vidsrc.to/embed/tv/${id}/${s}/${e}`;
  }
}
...
const TVPage: NextPage<{ show: any }> = ({ show }) => {
  ...
  const cast     = show.credits?.cast?.slice(0, 14) ?? [];
  ...
  return (
    <>
      <Head><title>{show.name} — StreamVault</title><meta name="description" content={show.overview?.slice(0, 160)} /></Head>
      <Navbar transparent />
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "#000", display: "flex", flexDirection: "column" as const }}>
          <div style={{ height: 50, background: "rgba(12,14,18,0.98)", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", padding: "0 16px", gap: 12, flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginLeft: "auto" }}>
              {/* Season/Episode quick selectors */}
              <select value={season} onChange={e => { setSeason(Number(e.target.value)); setEpisode(1); }}
                style={{ background: "#181c24", border: "1px solid rgba(255,255,255,0.1)", color: "#f1f5f9", padding: "4px 8px", borderRadius: 7, fontSize: 12 }}>
              ...
              {PLAYERS.map(p => (
                <button key={p.id} onClick={() => setPlayer(p.id)}
                  style={{ padding: "4px 11px", borderRadius: 7, border: `1px solid ${player === p.id ? p.color : "rgba(255,255,255,0.08)"}`, background: player === p.id ? `${p.color}20` : "transparent", color: player === p.id ? p.color : "#64748b", fontSize: 11, fontWeight: 700 }}>
                  {p.label}
                </button>
              ))}
            </div>
          </div>
          <iframe key={`${player}-${season}-${episode}`} src={embedUrl(player, show.id, season, episode)} allowFullScreen allow="autoplay;fullscreen" style={{ flex: 1, border: "none" }} />
        </div>
      )}

      <div style={{ position: "relative", minHeight: "90vh", background: "#0c0e12" }}>
        {show.backdrop_path && <img src={backdropUrl(show.backdrop_path, "original")} alt="" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.28, pointerEvents: "none" }} />}
        ...
        <div style={{ position: "relative", maxWidth: 1300, margin: "0 auto", padding: "120px 52px 60px", display: "flex", gap: 48, flexWrap: "wrap" as const, alignItems: "flex-start" }}>
          <img src={posterUrl(show.poster_path, "w342")} alt={show.name} onError={e => { (e.target as HTMLImageElement).src = "/placeholder-poster.jpg"; }} style={{ width: 220, borderRadius: 18, boxShadow: "0 24px 60px rgba(0,0,0,0.7)", border: "1px solid rgba(255,255,255,0.1)", flexShrink: 0 }} />
          <div style={{ flex: 1, minWidth: 260 }}>
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap" as const, marginBottom: 14 }}>
              {show.genres?.map((g: any) => <span key={g.id} style={{ background: "rgba(104,211,136,0.1)", color: "#68d388", fontSize: 11, fontWeight: 700, padding: "3px 12px", borderRadius: 20, border: "1px solid rgba(104,211,136,0.25)" }}>{g.name}</span>)}
            ...
            <p style={{ color: "#94a3b8", lineHeight: 1.78, marginBottom: 24, maxWidth: 520, fontSize: 14 }}>{show.overview}</p>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" as const }}>
                {PLAYERS.map(p => (
                  <button key={p.id} onClick={() => setPlayer(p.id)} style={{ padding: "6px 13px", borderRadius: 8, border: `1px solid ${player === p.id ? p.color : "rgba(255,255,255,0.1)"}`, background: player === p.id ? `${p.color}20` : "rgba(255,255,255,0.05)", color: player === p.id ? p.color : "#64748b", fontSize: 11, fontWeight: 700 }}>{p.label}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" as const, marginBottom: 14 }}>
                {seasons.map(s => <button key={s} onClick={() => { setSeason(s); setEpisode(1); }} style={{ width: 36, height: 36, borderRadius: 9, fontSize: 13, fontWeight: 700, border: `1px solid ${season === s ? "#f43a09" : "rgba(255,255,255,0.1)"}`, background: season === s ? "#f43a09" : "rgba(255,255,255,0.05)", color: season === s ? "#fff" : "#64748b" }}>{s}</button>)}
              </div>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" as const }}>
                {episodes.slice(0, 24).map(ep => <button key={ep} onClick={() => setEpisode(ep)} style={{ width: 36, height: 36, borderRadius: 9, fontSize: 12, fontWeight: 700, border: `1px solid ${episode === ep ? "#68d388" : "rgba(255,255,255,0.08)"}`, background: episode === ep ? "rgba(104,211,136,0.2)" : "rgba(255,255,255,0.04)", color: episode === ep ? "#68d388" : "#64748b" }}>{ep}</button>)}
              </div>
            </div>
          </div>
        </div>
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  try { const show = await tmdb.tvDetail(id); if (!show?.id) return { notFound: true }; return { props: { show } }; }
