// lib/auth.ts — NextAuth config. Works with or without env vars.
...
// Email/password — with demo fallback when no DB
providers.push(CredentialsProvider({
  ...
  async authorize(creds) {
    ...
    if (!hasDB()) {
      // Demo login when no DB configured
...
// DB Adapter — only when DATABASE_URL is set
  ...
  return {
    async createUser(u: any) {
      const [r] = await db`
        ...
        VALUES(${u.name},${u.email},${u.image??null},${u.emailVerified??null})
        ON CONFLICT(email) DO UPDATE SET name=EXCLUDED.name RETURNING *`;
      return r ?? u;
    },
    async getUser(id: string) {
      const [r] = await db`SELECT * FROM users WHERE id=${parseInt(id)}`;
      return r ?? null;
    },
    async getUserByEmail(email: string) {
      const [r] = await db`SELECT * FROM users WHERE email=${email}`;
      return r ?? null;
    },
    ...
    async updateUser(u: any) {
      const [r] = await db`UPDATE users SET name=${u.name},image=${u.image??null} WHERE id=${parseInt(u.id)} RETURNING *`;
      return r ?? u;
    },
    async linkAccount(a: any) {
      await db`
        ...
        VALUES(${a.userId},${a.type},${a.provider},${a.providerAccountId},
          ${a.access_token??null},${a.refresh_token??null},${a.expires_at??null},
          ${a.token_type??null},${a.scope??null},${a.id_token??null})
        ON CONFLICT DO NOTHING`;
    },
    async createSession(s: any) {
      const [r] = await db`
        INSERT INTO sessions(session_token,user_id,expires)
        VALUES(${s.sessionToken},${s.userId},${s.expires})
        ON CONFLICT(session_token) DO UPDATE SET expires=EXCLUDED.expires RETURNING *`;
      return r ?? s;
    },
    async getSessionAndUser(token: string) {
      ...
      if (!r) return null;
      return {
        session: { sessionToken:token, userId:String(r.uid), expires:r.expires },
        user:    { id:String(r.uid), name:r.name, email:r.email, image:r.image },
      };
    },
    async updateSession(s: any) {
      const [r] = await db`UPDATE sessions SET expires=${s.expires} WHERE session_token=${s.sessionToken} RETURNING *`;
      return r ?? null;
    },
    async deleteSession(token: string) {
      await db`DELETE FROM sessions WHERE session_token=${token}`;
    },
    async createVerificationToken(t: any) {
      const [r] = await db`
        INSERT INTO verification_tokens(identifier,token,expires)
        VALUES(${t.identifier},${t.token},${t.expires})
        ON CONFLICT DO NOTHING RETURNING *`;
      return r ?? t;
    },
    async useVerificationToken({ identifier, token }: any) {
      const [r] = await db`
        DELETE FROM verification_tokens WHERE identifier=${identifier} AND token=${token} RETURNING *`;
      return r ?? null;
    },
  };
}
